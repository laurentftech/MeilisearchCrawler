...
if metrics_text:
    ...
    col2.metric(t("api_metrics.google_cse"), f"{avg_cse_time:.2f} ms")
    col3.metric(t("api_metrics.reranking"), f"{avg_reranking_time:.2f} ms")
    ...
    st.subheader(t("api_metrics.live_api_metrics"))

    http_requests_total = metrics_data.get("http_requests_total", 0)
    http_requests_latency_avg = metrics_data.get("http_requests_latency_seconds_avg", 0) * 1000 # to ms

    col1, col2 = st.columns(2)
    col1.metric(t("api_metrics.total_http_requests"), f"{http_requests_total:.0f}")
    col2.metric(t("api_metrics.avg_latency"), f"{http_requests_latency_avg:.2f} ms")

    with st.expander(t("api_metrics.view_all_metrics")):
        df = pd.DataFrame(metrics_data.items(), columns=[t("api_metrics.metric"), t("api_metrics.value")])
        st.dataframe(df, use_container_width=True)

else:
    st.warning(t("api_metrics.no_metrics_warning"))

st.button(t("api_metrics.refresh_button"))
