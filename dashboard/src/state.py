...
def start_crawler(site=None, force=False, workers=None):
    ...
    try:
        ...
        st.error(f"Erreur lors du lancement du crawler: {e}") # On garde st.error pour les vraies exceptions
        return False

def stop_crawler():
    """Stops the running crawler process. Returns True on success, False on failure."""
    if not os.path.exists(PID_FILE):
        return False # Indique que le crawler ne tournait pas
...
def clear_cache():
    ...
    if os.path.exists(CACHE_FILE):
        ...
        return False # Pas de cache à vider
